...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_ENUM, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.skin, c.face, c.hairStyle, c.hairColor, c.facialStyle, "
                    "CAST(IFNULL(FLOOR((slotdata.head + slotdata.neck + slotdata.shoulders + slotdata.chest + slotdata.waist + slotdata.legs + slotdata.feet + slotdata.wrists + slotdata.hands + slotdata.finger1 + slotdata.trinket1 + slotdata.ranged + slotdata.back + slotdata.mainhand) / 14), 1) AS SIGNED) AS `level`, "
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT c.guid, account, name, race, class, gender, level, xp, money, skin, face, hairStyle, hairColor, facialStyle, bankSlots, restState, playerFlags, "
                     ...
                     "health, power1, power2, power3, power4, power5, power6, power7, instance_id, talentGroupsCount, activeTalentGroup, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, fishingSteps, talentLevel, lootPref, bonusTalents "
    ...
    // Instance saves
    PrepareStatement(CHAR_INS_INSTANCE_SAVE, "INSERT INTO instance (id, map, resettime, difficulty, completedEncounters, data, dungeonLevel) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,skin=?,face=?,hairStyle=?,hairColor=?,facialStyle=?,bankSlots=?,restState=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,online=?,talentLevel=?,lootPref=?,bonusTalents=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_GROUP_DUNGEON_LEVEL, "UPDATE `groups` SET dungeonLevel = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GROUP_RAID_DIFFICULTY, "UPDATE `groups` SET raiddifficulty = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GROUP_AFFIX, "UPDATE `groups` SET affix1 = ?, affix2 = ?, affix3 = ?, affix4 = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_DATA_INSTANCE_BY_INSTANCE, "DELETE FROM instance_data WHERE instanceId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_ACTION, "INSERT IGNORE INTO character_action (guid, spec, button, action, type) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_VIRTUAL_TEMPLATE, "REPLACE INTO item_template_virtual (entry, base_entry, itemguid, name, inventoryType, Quality, Flags, displayId, ItemLevel, StatsCount, "
        ...
        "statGroup, customFlags, sheath, legendaryId, generatedMagicFind, honeLevel) VALUES "
        "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_ACCOUNT_BANK_TAB, "INSERT INTO account_bank_tab (accountid, TabId) VALUES (?, ?)", CONNECTION_ASYNC); // 0: uint32, 1: uint8

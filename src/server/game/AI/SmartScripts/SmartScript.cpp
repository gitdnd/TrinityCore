    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SUMMON_CREATURE:
        {
            ...
            for (WorldObject* target : targets)
            {
                ...
                {
                    if (Creature* summon = summoner->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, Milliseconds(e.action.summonCreature.duration), privateObjectOwner, summoner->GetMap()->GetDungeonLevel()))
            ...
            for (uint32 counter = 0; counter < spawnsCount; counter++)
            {
                if (Creature* summon = summoner->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, Milliseconds(e.action.summonCreature.duration), privateObjectOwner, summoner->GetMap()->GetDungeonLevel()))

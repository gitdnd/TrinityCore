...
void ObjectMgr::LoadCreatureTemplateCustom(Field* fields)
{
    ...
    if (GetCreatureTemplate(entry))
    {
        ...
        if (QueryResult resistanceResults = WorldDatabase.PQuery("SELECT School, Resistance FROM creature_template_resistance where CreatureID = %u", entry))
        {
            do
            {
                Field* resFields = resistanceResults->Fetch();
                uint8 school = resFields[0].GetUInt8();
        ...
        if (QueryResult spellResults = WorldDatabase.PQuery("SELECT `Index`, Spell FROM creature_template_spell where CreatureID = %u", entry))
        {
            do
            {
                Field* spellFields = spellResults->Fetch();
                uint8 index = spellFields[0].GetUInt8();
                creatureTemplate.spells[index] = spellFields[1].GetUInt32();;
            } while (spellResults->NextRow());
        }
...
void ObjectMgr::LoadItemTemplate(uint32 entry)
{
    ...
    {
        _itemTemplateStore.rehash(_itemTemplateStore.size() + 1);
        uint32 entry = fields[0].GetUInt32();
        ItemTemplate& itemTemplate = _itemTemplateStore[entry];
...
void ObjectMgr::LoadVirtualItemTemplates()
{
    ...
    do
    {
        ...
        uint32 inventoryType = fields[i++].GetUInt32();
        if (inventoryType != 0)
        ...
        itemTemplate->Damage[0].DamageMin = fields[i++].GetFloat();
        itemTemplate->Damage[0].DamageMax = fields[i++].GetFloat();
        itemTemplate->Damage[0].DamageType = uint32(fields[i++].GetUInt8());
        itemTemplate->Delay = uint32(fields[i++].GetUInt16());
        itemTemplate->Armor = uint32(fields[i++].GetUInt16());
        itemTemplate->Bonding = uint32(fields[i++].GetUInt32());
        itemTemplate->Description = fields[i++].GetString();
        itemTemplate->Block = uint32(fields[i++].GetUInt32());
        itemTemplate->ItemSet = uint32(fields[i++].GetUInt32());
        for (uint8 j = 0; j < MAX_ITEM_PROTO_SOCKETS; ++j)
        ...
        for (uint8 j = 0; j < MAX_ITEM_PROTO_SPELLS; ++j)
        {
            itemTemplate->Spells[j].SpellId = fields[i++].GetUInt32();
            itemTemplate->Spells[j].SpellTrigger = fields[i++].GetUInt32();
            itemTemplate->Spells[j].SpellCharges = fields[i++].GetInt32();
            itemTemplate->Spells[j].SpellPPMRate = fields[i++].GetFloat();
            itemTemplate->Spells[j].SpellCooldown = fields[i++].GetInt32();
            itemTemplate->Spells[j].SpellCategory = fields[i++].GetUInt32();
...
void ObjectMgr::LoadInstanceTemplate()
{
    ...
    //                                                0     1       2        4
    QueryResult result = WorldDatabase.Query("SELECT map, parent, script, allowMount, maxPlayerOverride, mapXPRate, vLvlMod FROM instance_template");

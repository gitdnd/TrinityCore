...
void VirtualItemMgr::GenerateBaseStats(VirtualItemTemplate* output, VirtualModifier& modifier) const
{
    ...
    uint32 durability = round(float((output->ItemLevel * (output->Quality / 10)) + 25));
    debug << "ILvL " << output->ItemLevel << " Quality " << output->Quality << " Quality Mod " << float(output->Quality / 10) << " ilvl * quality " << float(output->ItemLevel * (output->Quality / 10)) << " final " << durability;

...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/, bool canBePersonal /* = true*/)
{
    ...
    ignoreMapLevels = store.GetName() == "item_loot_template" || store.GetName() == "spell_loot_template" || store.GetName() == "fishing_loot_template" || lootOwner->GetMap()->GetEntry()->IsContinent();
    if (ignoreMapLevels)

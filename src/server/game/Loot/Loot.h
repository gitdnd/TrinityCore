...
struct TC_GAME_API Loot
{
    ...
    bool FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError = false, uint16 lootMode = LOOT_MODE_DEFAULT, bool canBePersonal = true);

    // Inserts the item into the loot (called by LootTemplate processors)
    void AddItem(LootStoreItem const & item, VirtualModifier modifier = VirtualModifier(), bool canBePersonal = true);

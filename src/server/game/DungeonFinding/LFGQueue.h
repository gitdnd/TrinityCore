...
struct LfgQueueData
{
    ...
    LfgQueueData(time_t _joinTime, LfgDungeonSet const& _dungeons, LfgRolesMap const& _roles, int tanksNeeded, int healersNeeded, int dpsNeeded, bool _isSolo, int _itemLevel, int _penalty, uint32* _affixes) :
        joinTime(_joinTime),
        tanks(tanksNeeded),
        healers(healersNeeded),
        dps(dpsNeeded),
...
class TC_GAME_API LFGQueue
{
    public:
        ...
        void AddQueueData(ObjectGuid guid, time_t joinTime, LfgDungeonSet const& dungeons, LfgRolesMap const& rolesMap, uint32 itemLevel, uint32* affixes = 0);

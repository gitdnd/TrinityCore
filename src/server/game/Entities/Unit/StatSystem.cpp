...
{
    apply = _ModifyUInt32(apply, m_baseSpellHealingBonus, amount);
...
{
    apply = _ModifyUInt32(apply, m_baseSpellDamageBonus, amount);
...
void Player::UpdateDodgePercentage()
{
    if (!m_canDodge)
    {
        SetStatFloatValue(PLAYER_DODGE_PERCENTAGE, 0.0f);
        return;
    }
...
float Player::GetPowerRegen(Powers power) const
{
    ...
    if (power != POWER_MANA)
        regen += (power == POWER_ENERGY || !interrupted || power == POWER_FOCUS) ? powerRegenInfo[AsUnderlyingType(power)].first : 0.f;
...
void Guardian::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner && owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (IsRuneWeapon()) //Custom Dancing Rune Weapon
        {
            ...
            int32 holy = owner->GetInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + SPELL_SCHOOL_HOLY) - owner->GetInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + SPELL_SCHOOL_HOLY);

...
/*static*/ void Unit::CalcAbsorbResist(DamageInfo& damageInfo, Spell* spell /*= nullptr*/)
{
    ...
    if (Unit* attacker = damageInfo.GetAttacker())
    {
        ...
        auraAbsorbMod = std::max(auraAbsorbMod, static_cast<float>(attacker->GetMaxPositiveAuraModifier(SPELL_AURA_MOD_TARGET_ABILITY_ABSORB_SCHOOL, [&damageInfo](AuraEffect const* aurEff) -> bool
        {
            ...
            if (!aurEff->IsAffectingSpell(damageInfo.GetSpellInfo()) && (damageInfo.GetSpellInfo()->Id != 180088))
                return false;
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // Bonus from primary stats
        DoneAdvertisedBenefit += ToPlayer()->GetSpellPowerFromPrimaryStats();
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // Bonus from primary stats
        advertisedBenefit += ToPlayer()->GetSpellPowerFromPrimaryStats();
...
float Unit::MeleeSpellMissChance(Unit const* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId) const
{
    ...
    // Check if dual wielding, add additional miss penalty - when mainhand has on next swing spell, offhand doesnt suffer penalty - Except with Unrelenting Assault --itswicky
    if (!spellId && haveOffhandWeapon() && attType != RANGED_ATTACK && !m_currentSpells[CURRENT_MELEE_SPELL] && !HasAura(93178))
        missChance += 19.0f;
...
void Unit::OnDamageDealMakeThisAnAuraHookSometimeLater(Unit* victim, uint32& dmg, CleanDamage const* /*cleanDamage*/, DamageEffectType damagetype, SpellSchoolMask /*damageSchoolMask*/, SpellInfo const* spellProto)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_SCHOOL; ++i)
    {
        ...
        if (bonusdmgpct >= 0.01)
        {
            uint32 damage = std::floor((float(dmg) * float(bonusdmgpct / 100.0)));
...
void Unit::OnHealDealMakeThisAnAuraHookSometimeLater(Unit* victim, uint32& gain, SpellInfo const* spellProto)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_SCHOOL; ++i)
    {
        ...
        if (bonusdmgpct >= 0.01)
        {
            uint32 damage = std::floor((float(gain) * float(bonusdmgpct / 100.0)));
            CastSpellExtraArgs args;
            args.OriginalCaster = GetGUID();
            args.AddSpellBP0(damage);
            args.SetTriggerFlags(TRIGGERED_FULL_MASK);

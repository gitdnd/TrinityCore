...
bool Item::Create(ObjectGuid::LowType guidlow, uint32 itemId, Player const* owner, VirtualModifier modifier)
{
    ...
    {
        owner->Say("modifier level, owner level: " + std::to_string(modifier.plrAvgLvl) + ", " + std::to_string(owner->GetCappedItemLevel()), LANG_UNIVERSAL);
        ...
        if (Map* map = owner->GetMap())
            if ((map->IsDungeon() || map->IsRaid()) && map->GetPlayers().getSize() > 1)             // only apply if map, raid, and num players > 1, to avoid exploits
                if(modifier.ilevelBonus == 0)
...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid owner_guid, Field* fields, uint32 entry)
{
    ...
    uint32 durability = std::clamp((uint32)fields[8].GetUInt16(), (uint32)0, proto->MaxDurability);
...
bool Item::IsBoundByEnchant() const
{
    ...
    for (uint32 enchant_slot = PERM_ENCHANTMENT_SLOT; enchant_slot < MAX_ENCHANTMENT_SLOT; ++enchant_slot)
        if (uint32 enchant_id = GetEnchantmentId(EnchantmentSlot(enchant_slot)))
            if (SpellItemEnchantmentEntry const* enchantEntry = sSpellItemEnchantmentStore.LookupEntry(enchant_id))
                if (enchantEntry->Flags & ENCHANTMENT_CAN_SOULBOUND && enchant_id != 3729) // Fix for prismatic socket sending
                    return true;
    return false;
}
...
void Item::SaveVirtualItemInfo()
{
    if (VirtualItemTemplate const* itemTemplate = sVirtualItemMgr.GetVirtualTemplate(GetEntry()))
    {
        ...
        stmt->setUInt32(i++, itemTemplate->honeLevel);

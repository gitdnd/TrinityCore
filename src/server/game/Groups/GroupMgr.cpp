...
{
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT g.leaderGuid, g.lootMethod, g.looterGuid, g.lootThreshold, g.icon1, g.icon2, g.icon3, g.icon4, g.icon5, g.icon6"
            ...
            ", g.icon7, g.icon8, g.groupType, g.difficulty, g.raidDifficulty, g.masterLooterGuid, g.guid, lfg.dungeon, lfg.state, g.affix1, g.affix2, g.affix3, g.affix4 FROM `groups` g LEFT JOIN lfg_data lfg ON lfg.guid = g.guid ORDER BY g.guid ASC");
        if (!result)
        {
    ...
    TC_LOG_INFO("server.loading", "Loading Group instance saves...");
    {
        ...
        //                                                   0        1      2            3             4             5             6
        QueryResult result = CharacterDatabase.Query("SELECT gi.guid, i.map, gi.instance, gi.permanent, i.difficulty, i.resettime, i.dungeonLevel, "
            //           7
...
<<<<<<< HEAD
            InstanceSave* save = sInstanceSaveMgr->AddInstanceSave(mapEntry->MapID, fields[2].GetUInt32(), Difficulty(diff), fields[6].GetUInt32(), group->GetAffixData(1), group->GetAffixData(2), group->GetAffixData(3), group->GetAffixData(4), time_t(fields[5].GetUInt32()), fields[7].GetUInt64() == 0, true);
=======

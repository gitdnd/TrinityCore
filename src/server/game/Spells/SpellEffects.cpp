...
SpellEffectHandlerFn SpellEffectHandlers[TOTAL_SPELL_EFFECTS] =
{
    ...
    &Spell::EffectDodge,                                   // 20 SPELL_EFFECT_DODGE                    one spell: Dodge
...
void Spell::EffectPowerBurn()
{
    ...
    Powers powerType = Powers(effectInfo->MiscValue);
    if (!unitTarget || !unitTarget->IsAlive() || unitTarget->GetPowerType() != powerType && powerType != POWER_FOCUS || damage < 0)
        return;
...
void Spell::EffectEnergize()
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 94476:                                         // Replenishing Attacks
...
void Spell::EffectEnchantItemTmp()
{
    ...
    else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_CLASSLESS && m_spellInfo->SpellFamilyFlags[0] & 0x20000000)
        duration = 1800;                                    // 30 mins
...
void Spell::EffectWeaponDmg()
{
    ...
    case 14278: // Ghostly Strike
    {
        // Hemorrhage
        if (m_spellInfo->Id == 48660)
            AddComboPointGain(unitTarget, 1);
    ...
    case 48566: // Mangle (Cat)
    {
        AddComboPointGain(unitTarget, 1);
        break;
    }
    case 61006: // Kill Shot
    {
        spell_bonus += int32(0.4f * unitCaster->GetTotalAttackPowerValue(RANGED_ATTACK));
        break;
    }
...
void Spell::EffectSelfResurrect()
{
    ...
    player->SetFullPower(POWER_FOCUS);
...
void Spell::EffectActivateSpec()
{
    ...
    unitTarget->ToPlayer()->SetTalentLoadout(damage - 1);
}
...
void Spell::EffectReRollVirtualItemSockets()
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;
...
void Spell::EffectAddStatToVirtualItem()
{
    Player* player = m_caster->ToPlayer();
    if (!player)
        return;
...
void Spell::EffectCreateVirtualItem()
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;

    if (!sVirtualItemMgr.IsVirtualTemplate(sObjectMgr->GetItemTemplate(effectInfo->ItemType)))
        return;

    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
    uint32 itemId = effectInfo->ItemType;
    uint8 count = damage;
    Player* player = unitTarget->ToPlayer();
    ...
    modifier.statgroup = StatGroup(effectInfo->MiscValue);
    modifier.statpool = effectInfo->MiscValueB;
...
void Spell::EffectVirtualItemQualityUpgrade()
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;
    ...
    sVirtualItemMgr.UpdateDisenchantId(vItem, modifier);
...
void Spell::EffectReRollVirtualItem()
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;
...
void Spell::EffectHoneVirtualItem()
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;

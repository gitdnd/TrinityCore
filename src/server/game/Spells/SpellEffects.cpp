...
void Spell::EffectHoneVirtualItem()
{
    ...
    if (!roll_chance_f(honeChance))
    {
        ...
        {
            sWorld->SendGlobalText(Trinity::StringFormat("{} has broken their |c{}{}{}|Hitem:{}:0:0:0:0:0:0:0:0:0|h[{}]|h|r!", player->GetName(), std::hex, ItemQualityColors[ITEM_QUALITY_LEGENDARY],
                std::dec, itemTarget->GetEntry(), itemTarget->GetTemplate()->Name1).c_str(), nullptr, CHAT_MSG_LOOT);
        }
        return;
    }
